...
type SuratKeteranganWaliNikahProps = {
  tipe: String;
};

export default function SuratKeteranganWaliNikah({ tipe }: SuratKeteranganWaliNikahProps) {
  const initialData = {
    NamaPengaju: "",
    NIK: "",
  ...
  // 2. Mengumpulkan data spesifik dari form ke dalam satu objek
  const data_dinamis = {
    // Data dari bagian "Nama Pengaju"
    nama_pengaju: formData.NamaPengaju,
    nik_pengaju: formData.NIK,

    // Data dari bagian "Identitas Wali"
    nama_wali: formData.Nama,
    tempat_lahir_wali: formData.KabupatenLahir,
    tanggal_lahir_wali: formData.TanggalLahir,
    nik_wali: formData.NIK2,
    jenis_kelamin_wali: formData.JenisKelamin2,
    agama_wali: formData.Agama,
    pekerjaan_wali: formData.Perkerjaan,
    alamat_wali: formData.Alamat,
  };
  ...
  try {
    ...
    const res = await fetch("/api/permohonan", {
      ...
      body: JSON.stringify({
        ...
        nik: formData.NIK, // Menggunakan NIK pengaju sebagai identitas utama
        jenis_surat: "Surat Keterangan Wali Nikah",
        tipe: tipe,
        keterangan: `Pengajuan Surat Keterangan Wali Nikah oleh ${formData.NamaPengaju}`,
